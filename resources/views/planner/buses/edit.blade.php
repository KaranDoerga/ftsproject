<x-app-layout>
    <x-slot name="header">
        <h2 class="font-semibold text-xl text-gray-800 dark:text-gray-200 leading-tight">
            Bus Bewerken: {{ $bus->type }} ({{ $bus->license_plate ?? 'Geen kenteken' }})
        </h2>
    </x-slot>
        ...
        <div class="max-w-3xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white dark:bg-gray-800 overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6 text-gray-900 dark:text-gray-100">
                    <form method="POST" action="{{ route('planner.buses.update', $bus->id) }}">
                        @csrf
                        @method('PUT')

                        <div>
                            <label for="type" class="block font-medium text-sm text-gray-700 dark:text-gray-300">Type Bus</label>
                            <input id="type" class="block mt-1 w-full rounded-md shadow-sm border-gray-300 dark:border-gray-700 dark:bg-gray-900 dark:text-gray-300 focus:border-indigo-500 dark:focus:border-indigo-600 focus:ring-indigo-500 dark:focus:ring-indigo-600" type="text" name="type" value="{{ old('type', $bus->type) }}" required autofocus />
                            @error('type') <span class="text-red-500 text-sm">{{ $message }}</span> @enderror
                        </div>

                        <div class="mt-4">
                            <label for="capacity" class="block font-medium text-sm text-gray-700 dark:text-gray-300">Capaciteit</label>
                            <input id="capacity" class="block mt-1 w-full rounded-md shadow-sm border-gray-300 dark:border-gray-700 dark:bg-gray-900 dark:text-gray-300 focus:border-indigo-500 dark:focus:border-indigo-600 focus:ring-indigo-500 dark:focus:ring-indigo-600" type="number" name="capacity" value="{{ old('capacity', $bus->capacity) }}" required min="1" />
                        ...
                        <div class="mt-4">
                            ...
                            <input id="driver" class="block mt-1 w-full rounded-md shadow-sm border-gray-300 dark:border-gray-700 dark:bg-gray-900 dark:text-gray-300 focus:border-indigo-500 dark:focus:border-indigo-600 focus:ring-indigo-500 dark:focus:ring-indigo-600" type="text" name="driver" value="{{ old('driver', $bus->driver) }}" />
                        ...
                        <div class="mt-4">
                            ...
                            <select id="available" name="available" class="block mt-1 w-full rounded-md shadow-sm border-gray-300 dark:border-gray-700 dark:bg-gray-900 dark:text-gray-300 focus:border-indigo-500 dark:focus:border-indigo-600 focus:ring-indigo-500 dark:focus:ring-indigo-600" required>
                                <option value="1" {{ old('available', $bus->available) == 1 ? 'selected' : '' }}>Ja</option>
                                <option value="0" {{ old('available', $bus->available) == 0 ? 'selected' : '' }}>Nee</option>
                            </select>
